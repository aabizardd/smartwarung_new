    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            data +=
                ...
                '<dd class="col-sm-8">' + response[0]['invoice_id'] +
                '<button class="btn btn-info float-right" onclick="printDiv();">Cetak</button></dd>'
            ...
            for (i = 0; i < length; i++) {
                data +=
                    ...
                    '<div class="col-sm-4"><img width="25%" src="<?php echo base_url('assets/uploads/') ?>' +
                    response[i]['item_photo'] + '" alt=""></div>' +
                    '<div class="col-sm-4 pl-2" style="padding-top:2%">' +
                    '<span><a href="<?php echo site_url('item/show/') ?>' + response[i]['item_id'] +
                    '" class="font-weight-bold">' + response[i]['item_name'] + '</a></span><br>' +
                    '<span>' + response[i]['details_quantity'] + ' x ' + formatNumber(response[i][
                        'details_price'
                    ]) + '</span>' +
                    '</div>' +
                    ...
                    '<span>' + formatNumber(response[i]['details_quantity'] * response[i][
                        'details_price'
                    ]) + '</span>' +
                    '</div>';
                if (response[i]['invoice_status'] === "Sudah diterima") {
                    data += '<a href="<?php echo site_url('rating/create/') ?>' + response[i]['item_id'] +
                        '/' + response[i]['invoice_id'] +
                        '" class="mt-2 ml-auto btn btn-sm btn-primary px-3 text-white">Beri Ulasan</a>' +
                        '</div>';
                } else {
                    data += '</div>';
            ...
            if (response[0]['invoice_status'] === "Menunggu proses penjual") {
                dataFooter += '<a href="<?php echo site_url('invoice/cancel/') ?>' + response[0][
                    'invoice_id'
                ] + '" class="btn btn-sm btn-danger px-3" onclick="' + confirm + '">Batalkan</a>';
            } else if (response[0]['invoice_status'] === "Sedang dikirim") {
                dataFooter += '<a type="button" href="<?php echo site_url('invoice/update_to_done/') ?>' +
                    response[0]['invoice_id'] + '" class="btn btn-sm btn-primary px-3">Terima barang</a>';
            ...
            if (response[0]['invoice_status'] === "Menunggu proses penjual") {
                dataFooter += '<a href="<?php echo site_url('invoice/cancel/') ?>' + response[0][
                    ...
                    '<a href="<?php echo site_url('invoice/update_to_process/') ?>' + response[0][
                        'invoice_id'
                    ] + '" class="btn btn-sm btn-primary px-3" >Proses dan kirimkan</a>';
            } else if (response[0]['invoice_status'] === "Sedang dikirim") {
                dataFooter +=
                    '<button type="button" class="btn btn-sm btn-secondary px-3" data-dismiss="modal">Close</button>';
            } else if (response[0]['invoice_status'] === "Sudah diterima") {
